...
- 设置：GitHub Token、GitHub 代理、安装根目录、界面语言、主题、后台检查、开机后后台启动、通知权限操作、检查间隔和下载加速。
- 系统托盘：关闭窗口后驻留托盘、一致的取消最小化/聚焦恢复、单实例恢复窗口、手动检查、恢复窗口、退出和更新数量提示。
...
本地托管更新使用 staging 和 rollback snapshot，替换失败时可以保留或恢复旧版本。系统安装器会保留可追踪信息：ReleaseDock 记录安装包路径，并且在 Windows 上会在安装后、dashboard 刷新或 Inspector 手动重新检测时，自动从系统卸载注册表重新探测真实安装目录。
