...
const copy: Record<Language, Copy> = {
  en: {
    ...
    proxyUrlHelp: "Format: http://host:port or https://host:port. Overrides the Windows system proxy, PAC, or WPAD policy for GitHub queries and Release asset downloads only.",
    ...
    networkConfigHealthHelp: "ReleaseDock automatically follows the Windows system proxy, PAC, or WPAD policy when no proxy is entered here. Token is optional for public repos and useful for private repos or rate limits.",
    ...
    connectivityTestHelp: "Use the configured proxy or the Windows system network policy, plus the optional token, to test GitHub API reachability.",
    ...
    connectivityNetworkFailureHelp: "Check direct GitHub access and the Windows system proxy, PAC, or WPAD policy. If neither provides a trusted route, contact your network administrator.",
  ...
  "zh-CN": {
    ...
    proxyUrlPlaceholder: "http://proxy.example.com:port",
    proxyUrlHelp: "格式：http://主机:端口 或 https://主机:端口。此处填写后会覆盖 Windows 系统代理、PAC 或 WPAD 策略，只影响 GitHub 查询和 Release 资产下载。",
    ...
    networkConfigHealthHelp: "此处未填写代理时，ReleaseDock 会自动遵循 Windows 系统代理、PAC 或 WPAD 策略；公开仓库无需 Token，私有仓库或限流时再配置。",
    ...
    connectivityTestHelp: "使用已配置代理或 Windows 系统网络策略，以及可选 Token，测试 GitHub API 是否可访问。",
    ...
    connectivityNetworkFailureHelp: "先检查 GitHub 官方直连以及 Windows 系统代理、PAC 或 WPAD 策略；如均不能提供受信任路径，请联系网络管理员。",
